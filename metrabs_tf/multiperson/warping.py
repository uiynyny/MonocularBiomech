...
class TFAFake:
    class image:
        @staticmethod
        def transform(image, H, output_shape, interpolation='bilinear'):
            import tensorflow as tf
            return tf.raw_ops.ImageProjectiveTransformV3(
                images=tf.expand_dims(image, 0),
                transforms=tf.expand_dims(H, 0),
        ...
        @staticmethod
        def interpolate_bilinear(images, query_points, indexing='ij'):
            import tensorflow as tf
            # Return dummy tensor to allow tf.cond tracing
            batch_size = tf.shape(query_points)[0]
            num_points = tf.shape(query_points)[1]
            channels = tf.shape(images)[-1]
            return tf.zeros([batch_size, num_points, channels], dtype=images.dtype)
